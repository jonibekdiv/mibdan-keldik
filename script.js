...
const UZBEKISTAN_REGIONS = {
  ...
  'Toshkent viloyati': ['Angren', 'Bekobod', "Bo'ka", "Bo'stonliq", 'Chinoz', 'Ohangaron', "Oqqo'rg'on", 'Parkent', 'Piskent', 'Quyi Chirchiq', 'Toshkent', 'Yuqori Chirchiq', 'Zangiota', "Yangiyo'l"],
...
function loadProfile() {
  try {
    ...
    console.warn("Profilni yuklashda xatolik:", error);
  }
}
...
function renderNotifications() {
  const overdue = state.debtors.filter(debtor =>
    debtor.dueDate &&
    getDaysLeft(debtor.dueDate) < 0 &&
    debtor.status !== 'completed' &&
    Math.max(Number(debtor.amount || 0) - Number(debtor.totalPaid || 0), 0) > 0
  );
...
document.getElementById('debtorForm').addEventListener('submit', (e) => {
  ...
  if (state.currentEditId) {
    ...
    if (d) {
      Object.assign(d, { fullName: name, phone, address, province, district, mahalla, amount, loanDate, dueDate, status, notes, updatedAt: new Date().toISOString() });
    }
    showToast('✅ Yangilandi', 'success');
  } else {
    state.debtors.push({
      id: generateId(), fullName: name, phone, address, province, district, mahalla, amount,
      loanDate, dueDate, status, notes, totalPaid: 0, payments: [],
...
function openPaymentModal(debtorId) {
  ...
  if (!d) return;
  const remaining = Math.max((d.amount || 0) - (d.totalPaid || 0), 0);
...
document.getElementById('paymentForm').addEventListener('submit', function (e) {
  e.preventDefault();
  const d = state.debtors.find(x => x.id === state.currentDebtorId);
  if (!d) return;
  ...
  const remaining = (d.amount || 0) - (d.totalPaid || 0);
  if (amount > remaining) {
  ...
  d.payments.push({ id: generateId(), amount, date, method, createdAt: new Date().toISOString() });
...
function openDetailsModal(debtorId) {
  ...
  const statusText = d.status === 'completed' ? "To'liq to'langan"
    : d.status === 'partial' ? "Qisman to'langan"
    : d.status === 'overdue' ? "Muddati o'tgan" : 'Faol';
  ...
  let html = `
    <div class="details-hero">
      <div class="details-avatar"><i data-lucide="user-round"></i></div>
      <div><strong>${escapeFrontend(d.fullName)}</strong><span>${statusText}</span></div>
    </div>
    <div class="details-section">
      <div class="section-title"><i data-lucide="contact"></i> Aloqa</div>
      <div class="detail-grid">
        <div class="detail-row"><span class="label"><i data-lucide="phone"></i> Telefon</span><span>${escapeFrontend(d.phone)}</span></div>
        <div class="detail-row"><span class="label"><i data-lucide="map-pin"></i> Manzil</span><span>${escapeFrontend(d.address || "Ko'rsatilmagan")}</span></div>
    ...
    ${d.notes ? `<div class="details-note"><i data-lucide="notebook-pen"></i><span>${escapeFrontend(d.notes)}</span></div>` : ''}
  ...
  if (d.payments && d.payments.length) {
    d.payments.slice().reverse().forEach(p => {
      html += `<div class="payment-item"><div><strong>${formatCurrency(p.amount)}</strong><span>${formatDate(p.date)} · ${methodNames[p.method] || p.method || "Usul ko'rsatilmagan"}</span></div></div>`;
    });
  } else {
...
function renderDebtors() {
  ...
  if (term) {
    filtered = filtered.filter(d =>
      (d.fullName || '').toLowerCase().includes(term) || (d.phone || '').includes(term)
    );
  }

  if (state.filterType !== 'all') {
    if (state.filterType === 'overdue') {
      filtered = filtered.filter(d => getDaysLeft(d.dueDate) < 0 && d.status !== 'completed');
    } else {
  ...
  else if (state.sortBy === 'amount') filtered.sort((a, b) => (b.amount || 0) - (a.amount || 0));
  ...
  list.innerHTML = filtered.map(d => {
    const remaining = (d.amount || 0) - (d.totalPaid || 0);
    const progress = d.amount > 0 ? ((d.totalPaid || 0) / d.amount) * 100 : 0;
    ...
    return `
      <div class="debtor-card">
        <div class="debtor-header">
          <span class="debtor-name">${escapeFrontend(d.fullName)}</span>
        ...
        <div class="debtor-info">
          <div><span class="label"><i data-lucide="phone"></i></span> ${escapeFrontend(d.phone)}</div>
        ...
        <div class="paid-line">To'langan: ${formatCurrency(d.totalPaid)} / ${formatCurrency(d.amount)}</div>
        ${daysText}
...
function updateUI() {
  ...
  const totalDebt = state.debtors.reduce((s, d) => s + Number(d.amount || 0), 0);
  const totalPaid = state.debtors.reduce((s, d) => s + Number(d.totalPaid || 0), 0);
...
function updateCharts() {
  ...
  chartStatus = new Chart(ctx1, {
    ...
    data: {
      ...
      datasets: [{
        data: [statuses.active, statuses.partial, statuses.completed, statuses.overdue],
      ...
      scales: {
        ...
        y: {
          ...
          ticks: {
            color: textColor,
            font: chartFont,
            callback: value => new Intl.NumberFormat('uz-UZ', { notation: 'compact' }).format(value)
          }
        }
      }
    }
...
document.getElementById('addDemoBtn').addEventListener('click', () => {
  ...
  const demo = [
    { fullName: 'Ali Karimov', phone: '+998 90 123 45 67', address: 'Toshkent', amount: 5000000, loanDate: iso(d1), dueDate: iso(d3), status: 'overdue', notes: "Muddati o'tgan", totalPaid: 2000000, payments: [{ id: generateId(), amount: 2000000, date: iso(new Date(now.getTime() - 3 * 86400000)), method: 'cash', createdAt: now.toISOString() }] },
    { fullName: 'Gulnora Akbarova', phone: '+998 91 234 56 78', address: 'Samarqand', amount: 3500000, loanDate: iso(new Date(now.getTime() - 15 * 86400000)), dueDate: iso(d2), status: 'partial', notes: '', totalPaid: 1500000, payments: [{ id: generateId(), amount: 1000000, date: iso(new Date(now.getTime() - 10 * 86400000)), method: 'card', createdAt: now.toISOString() }, { id: generateId(), amount: 500000, date: iso(new Date(now.getTime() - 5 * 86400000)), method: 'transfer', createdAt: now.toISOString() }] },
    { fullName: 'Jasur Maxmudov', phone: "+998 99 345 67 89", address: "Farg'ona", amount: 2000000, loanDate: iso(new Date(now.getTime() - 60 * 86400000)), dueDate: iso(new Date(now.getTime() - 10 * 86400000)), status: 'completed', notes: "To'liq to'langan", totalPaid: 2000000, payments: [{ id: generateId(), amount: 2000000, date: iso(new Date(now.getTime() - 8 * 86400000)), method: 'transfer', createdAt: now.toISOString() }] },
    { fullName: 'Sonya Uzbekova', phone: '+998 88 456 78 90', address: 'Andijon', amount: 4500000, loanDate: iso(now), dueDate: iso(d4), status: 'active', notes: 'Yangi qarz', totalPaid: 0, payments: [] }
  ];
  demo.forEach(d => { d.id = generateId(); d.createdAt = now.toISOString(); d.updatedAt = now.toISOString(); });
  state.debtors = demo;
  saveData();
  updateUI();
  renderDebtors();
...
document.getElementById('clearAllBtn').addEventListener('click', () => {
  if (confirm("Barcha ma'lumotlar o'chiriladi. Davom?")) {
    ...
    renderDebtors();
    closeModal('settingsModal');
    showToast("🗑 Hammasi o'chirildi", 'success');
  }
});
...
document.getElementById('exportExcelBtn').addEventListener('click', () => {
  ...
  state.debtors.forEach(d => {
    const rem = (d.amount || 0) - (d.totalPaid || 0);
    csv += `${d.fullName},${d.phone},"${(d.address || '').replace(/"/g, '""')}",${d.amount},${d.totalPaid},${rem},${d.status},${d.dueDate}\n`;
  });
